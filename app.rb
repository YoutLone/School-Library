...
class App
  ...
  def create_student_file(name, age, permisson)
    @people << Student.new(age, name, parent_permission: permisson)
  end
  ...
  def create_teacher_file(name, age, specialization)
  ...
  def create_book_file(title, author)
    @book << Book.new(title, author)
  end
  ...
  def create_rental_file(date, person_index, book_index)
    @rentals << Rental.new(date, @people[person_index], @book[book_index])
  end
